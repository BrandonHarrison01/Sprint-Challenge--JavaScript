...
const universities = graduates.map(function(person){
...
const uni = graduates.map(function(univers){
  if (univers.university.includes("Uni")) {
    return univers.university
  }
});
console.log(uni);
...
*/
const animalNames = zooAnimals.forEach((animal) => console.log(animal.animal_name, animal.scientific_name));
...
const lowerCase = zooAnimals.map((lowName) => lowName.animal_name.toLowerCase());
